...
实际项目将 Excel、统计、图表、LLM 解读和 Word/PDF 交付连接起来。这里使用全新编写的 12 条“处理任务”数据，演示输入到结果的核对方式。合成字段和计算脚本为本次原创缩小示例，与系统生产代码分别维护。
...
预先编写的示例解读：纳入统计的11条记录中，9条已完成。C组4条全部完成，A组和B组各有1条待完成。样例量小，仅展示计算与表达方式。

## 4. 交付：把规则、图表与结论放在同一页

![原创交付样式示意，非生产系统输出](../assets/report-layout.png)

上图是本次重新绘制的交付样式示意，使用本页实算结果。它用于说明报告应保留什么信息；未运行系统的 LLM、Quarto 或 Windows Word 生产链。
...
我在项目中的一次经历：校级报告验收时发现分析被简化为4个逻辑图，要求先审核完整方案，再推进生产。确认10个图号后，借助AI开发工具统一图表、章节解读、模板和检查规则。

完整结构涵盖整体表现（图1—4）、年级与班级比较（图5—7）、证书等级与学习过程分组（图8—10）。10个逻辑图号代表分析主题，拆分物理图仍使用同一图号。该过程体现需求判断与验收责任。

本次曾在私有工作副本核对图义契约和相应测试入口；公开仓库不含当时完整对话或内部实现。上述历史过程按我的工作回顾呈现，本页合成脚本单独证明缩小计算示例可运行。
...
| 交付样式 | 本次原创示意，未走系统生产链 |
| 累计约50份正式报告 | 来自历次迭代的实际使用回顾，没有在这里逐一复点历史交付 |
| 约5个工作日到2—3小时 | 实际使用经验估计，包含制作与复核；未做配对计时 |
| 系统生产依赖 | Windows、Microsoft Word、Quarto与配置好的LLM |
| 系统历史测试 | 不作为本次测试；当前副本回归受环境依赖阻塞 |
